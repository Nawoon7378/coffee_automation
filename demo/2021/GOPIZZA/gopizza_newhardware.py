...
# vision_client = Receiver("192.168.0.0", "50009") #receiver 객체 생성 부탁드립니다. 
...
# def pixel_to_meter(pixel_int):
#     return float(pixel_int)*0.001
...
#             pizza_pos = [
#                 pixel_to_meter(pizza_dict['center_point']['x']), 
#                 pixel_to_meter(pizza_dict['center_point']['y']), 
#                 0, 
#                 0, 
#                 180, 
#                 pizza_dict['angle']
...
# TODO 
# 작업단계에 따른 초기화 모션 변경( 사용자가 정지 -> 시작 버튼 클릭시 )
...
def state_manager_thr():
    ...
    # for test
    indy_master.write_direct_variable(0, RESERVE_PIZZA_ADDR, 1)
    ...
    while GFLAG['run']:
        ...
        if indy_master.read_direct_variable(0, REFERENCE_UPDATE_CMD) == 1:
            indy_master.set_reference_frame(STATE['recent_pizza_pos'])
            sleep(0.05)
            indy_master.write_direct_variable(0, REFERENCE_UPDATE_CMD, 0)

        # reference frame reset
        if indy_master.read_direct_variable(0, REFERENCE_UPDATE_CMD) == 2:
        ...
        # collision level reset
        if indy_master.read_direct_variable(0, CUTTING_MOTION_COLLISION_ADDR) == 2:
...
def pusher_thr():
    while GFLAG['run']:
        ...
        if (
            ...
            STATE['recent_pizza_cat'] = 1
            STATE['recent_pizza_pos'] = [0.1, 0.1, 0, 0, 180, 5]

            logging.info("cat : {}, pos : {}".format(STATE['recent_pizza_cat'], STATE['recent_pizza_pos']))

            if STATE['recent_pizza_cat'] is None: # 데이터 받는데 실패하는경우
                STATE['run'] = False # 모두 종료
                indy_master.stop_current_program()
                break

            indy_master.write_direct_variable(0, PIZZA_CATEGORY, STATE['recent_pizza_cat'])
