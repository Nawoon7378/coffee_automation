...
vision_client = Receiver("192.168.0.2", "50009") #receiver 객체 생성 부탁드립니다. 
...
def commander():
    ...
    reference_pos = [0.27413975958562303, -0.6623553350139346, -0.002, -178.90482652993092, 1.729263648486311, 90.7587314727181]
    while GFLAG['run']:
        if indy_master.read_direct_variable(0, CUTTING_MOTION_ADDR) == 1:
            ...
            sleep(0.1)
            indy_master.task_move_to(get_new_pos([-0.0026, 0.0228, -0.0079, 15.5955, -0.2962, 0.0667])) # tilt 1
            indy_master.task_move_to(get_new_pos([0, 0, 0, 0, 0, 0])) # cut start
            indy_master.task_move_to(get_new_pos([0.0012, -0.0327, -0.0055, 163.4746, 179.9115, 179.9896])) # tilt 2
